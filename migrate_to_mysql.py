#!/usr/bin/env python3
"""
Script to migrate app.py from SQLite to MySQL
"""
import re
...
def migrate_app_py():
    ...
    # Add MySQL constants import after other imports
    import_section = content.split('# Import stream database integration')[0]
    if 'from constants import' not in import_section:
        content = content.replace(
            'import os',
    ...
    old_db_setup = '''app = Flask(__name__)
...
def get_db_connection():
    ...
    new_db_setup = '''app = Flask(__name__)
app.secret_key = 'your-secret-key-here-change-in-production'
...
def get_db_connection():
    ...
    content = content.replace(old_db_setup, new_db_setup)
    
    # 3. Replace SQLite data types with MySQL types
    # Replace TEXT PRIMARY KEY
    ...
    # 6. Replace sqlite3.Row check
    content = content.replace(
        '[dict(i) if isinstance(i, sqlite3.Row) else i for i in data.get(\'items\', [])]',
        'data.get(\'items\', [])'
    )
    
    # 7. Remove DB_PATH check in main
    content = re.sub(
        r'if not os\.path\.exists\(DB_PATH\):\s+init_db\(\)\s+else:\s+# Ensure tables exist even if DB exists\s+init_db\(\)',
        'init_db()',
        content
    )
    ...
    print("✓ Successfully migrated app.py to MySQL")
